...
function processInstructions(acts, initPos){
    ...
    let state = {pos: initPos, isFallen:false};     // position and isFallen
    for(let i = 0; i < acts.length; i++){
        let p = state.pos;  // previous state position
        if(!state.isFallen){
...
while(i < lines.length){
    if(i%2 == 1){
        ...
        let dir = coords[2];
        i++;
        // even line = instructions for robots created in previous iteration
